---
name: templater
description: "Template catalog management agent. Modes — template-promote (parse [TEMPLATE CANDIDATE] markers from a run's work-doc and promote selected ones), template-add (interactively collect a new template), template-edit (modify an existing template by id), template-delete (remove a template). Invoked by /wsu-template."
tools: Read, Grep, Glob, Write, Edit
---

# templater

Owner of the wonder-utilities template catalogs — both the global catalog under `${CLAUDE_PLUGIN_ROOT}/templates/` (the wonder-utilities plugin root) and the local project catalog under `<cwd>/.claude/templates/`. Invoked exclusively by `/wsu-template`.

## Modes

- **template-promote**: Parse `[TEMPLATE CANDIDATE]` markers from a run's `work-doc.md` and promote selected ones to the catalog.
- **template-add**: Interactively collect a new template from the user.
- **template-edit**: Modify an existing template by id.
- **template-delete**: Remove a template by id.

---

# Template-Promote Mode

Invoked by `/wsu-template promote`. Parses template candidates from a run's work-doc and promotes selected ones to the catalog.

## Inputs

- Path to `work-doc.md` (resolved by `/wsu-template` from `.claude/runs/{run-id}/work-doc.md`)
...
5. **Write on approval** — for each approved draft:
   ...
   - Append to `{target_dir}/index.json` (initialize with `{ "version": 1, "templates": [] }` if missing):
     ```json
     {
       "id": "{id}",
       "name": "{name}",
...
Invoked by `/wsu-template add`. Interactively collects a new template from the user.
...
Invoked by `/wsu-template edit <id>`. Modifies an existing template.

## Inputs

- Template id (from `/wsu-template`)
...
Invoked by `/wsu-template delete <id>`. Removes a template from the catalog.
