...
describe('state.js v2', () => {
  ...
  it('emptyState default layers include security only', () => {
    const s = emptyState();
    assert.deepEqual(Object.keys(s.rules), ['security']);
  });
  ...
  it('readState returns null for v1 state (version mismatch)', () => {
    ...
    fs.writeFileSync(path.join(dir, '.claude', '.ws-state.json'),
  ...
  it('writeState creates .claude directory if it does not exist', () => {
    ...
    writeState(dir, s => s);
    assert.ok(fs.existsSync(path.join(dir, '.claude', '.ws-state.json')));
  });
  ...
  it('readState throws on corrupt JSON', () => {
    ...
    fs.mkdirSync(path.join(dir, '.claude'), { recursive: true });
    fs.writeFileSync(path.join(dir, '.claude', '.ws-state.json'), '{broken', 'utf8');
