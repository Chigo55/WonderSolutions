---
description: "Structural Layering Meta-Rules"
activation: "always"
...
> Related meta-rules: `${CLAUDE_PLUGIN_ROOT}/rules/security.md` · `${CLAUDE_PLUGIN_ROOT}/rules/workflow.md`
> Generated output location: `.antigravity/rules/{layer}.md` (for each declared layer)
...
- **레이어 자율 선언**: 프로젝트의 구조적 복잡성(단일 스크립트, CLI, 웹 API, 복합 앱 등)에 맞추어 필요한 레이어를 선언하고 개별 룰(`.antigravity/rules/{layer}.md`)로 작성합니다.
...
생성되는 모든 개별 레이어 규칙 파일(`.antigravity/rules/{layer}.md`)은 다음 필수 섹션을 포함해야 합니다:
...
#### Entry-Points & I/O Contract
- 진입점은 stdin 또는 CLI 인수에서 데이터를 읽어 `JSON.parse` / `process.argv`로 파싱하는 스크립트 형태임.
- 출력은 호출자와 합의된 단일 포맷(예: JSON 객체 또는 종료 코드)으로 한정하며, 정상 종료는 `process.exit(0)`, 실패 종료는 `process.exit(1)`을 사용해야 함.

#### Core Logic & Side-Effects
- 로직 모듈(순수 함수)은 I/O를 수행하지 않아야 함.
- 모든 I/O 작업은 진입 스크립트와 상태 모듈에 격리함.

#### State & Persistence
- 데이터 저장소는 단일 로컬 JSON 파일(예: `.antigravity/<tool>-state.json`)로 한정함.
...
개별 레이어 규칙 파일 `.antigravity/rules/{layer}.md` 작성이 완료된 후 ruler는 다음 사항을 검증합니다:
