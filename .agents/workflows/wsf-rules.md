...
5. On approval, ruler writes the updated `.antigravity/rules/{layer}.md` and appends amendment log to `.antigravity/adr/{layer}.md`.

## Audit mode

1. Invoke **ruler** in **audit mode**.
2. Ruler reads all `.antigravity/rules/*.md` and `.antigravity/adr/*.md`.
3. Ruler writes `.antigravity/reports/wsf-rules-audit-{YYYYMMDD-HHMMSS}.md`.
