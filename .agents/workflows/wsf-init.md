---
name: "wsf-init"
description: "Initializes wonder-workflows for a project — provisions the ws-state.antigravity.json registry, reverse-engineers ADRs, generates project-specific rules, and produces HTML reports. Run once per project before using /wsf-run."
---
...
## 1. Provision the State Registry (`ws-state.antigravity.json`)
...
- **Invalid JSON** (parse failure) → rename the corrupt file to `ws-state.antigravity.json.bak` (replacing any older `.bak`), then build a fresh registry from the scan and write it. Tell the user the backup path.
...
```json
{
  "project": "{project name} (Antigravity)",
  "version": "1.0.0",
  "plugins": {
    "wonder-workflows": {
...
After writing, report one line: `ws-state.antigravity.json provisioned — wonder-utilities: {detected|absent} · wonder-plugins: {detected|absent}`.
...
4. Write `.antigravity/adr/{layer}.md`

After ruler confirms the ADR file (`.antigravity/adr/{layer}.md`) is written, proceed to Step 2.
...
2. Load `.antigravity/adr/{layer}.md` (required — ruler will abort if absent)
...
5. Write `.antigravity/rules/{layer}.md`

After ruler confirms the rule file (`.antigravity/rules/{layer}.md`) is written, proceed to Step 3.
...
Write the report to `.antigravity/reports/wsf-init-{layer}-YYYYMMDD-HHMMSS.html`.
...
<body>
  ...
  <h2>생성된 규칙</h2>
  <pre>{full content of .antigravity/rules/{layer}.md}</pre>
  <h2>ADR ↔ 규칙 매핑</h2>
  <table>
...
Generated:
  ✓ ws-state.antigravity.json — state registry (wonder-utilities: {detected|absent} · wonder-plugins: {detected|absent})
  ✓ {layer-name-1} — .antigravity/adr/{layer-name-1}.md, .antigravity/rules/{layer-name-1}.md, .antigravity/reports/wsf-init-{layer-name-1}-YYYYMMDD-HHMMSS.html
  ✓ {layer-name-2} — ...
  ...
...
Open .antigravity/reports/ to review the initialization reports.
```

## Overwrite Policy

Before Step 1 for each layer, check whether `.antigravity/adr/{layer}.md` or `.antigravity/rules/{layer}.md` already exists.

- If either exists, ask:
  > "`.antigravity/adr/{layer}.md` and/or `.antigravity/rules/{layer}.md` already exist. Overwrite or skip? (overwrite / skip)"
