...
> Related meta-rules: `.codex/wonder/meta-rules/security.md`
> Plugin: wonder-utilities (WonderSolutions suite)
> Generated output location: `.codex/wonder/rules/templates.md`
...
생성되는 `.codex/wonder/rules/templates.md` 파일은 반드시 아래의 필수 섹션을 포함해야 합니다:
...
### Catalog Summary
- `.codex/wonder/templates/index.json` (wonder-utilities 글로벌 카탈로그) 및 `<cwd>/.codex/wonder/templates/index.json` (로컬 카탈로그, 선택적) 파일을 분석하여 등록되어 있는 템플릿 명세(id, tags, description)를 로드합니다.
...
템플릿 규칙 파일 `.codex/wonder/rules/templates.md` 작성이 끝난 후 templater는 다음 사항을 최종 대조합니다:
...
- [ ] No contradictions with `$wonder-template` command behavior
